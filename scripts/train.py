...
parser.add_argument(
    ...
    type=float,
    default=None,
    help="Stop training early if mean reward exceeds this value (default: None, disabled)",
)
...
if __name__ == "__main__":
    ...
    args.recurrent = args.recurrence > 1
    ...
    # Load model
    # acmodel = ACModel(obs_space, envs[0].action_space, args.recurrent, args.text)
    acmodel = ModelFreeACModel(
        obs_space, envs[0].action_space, args.recurrent, args.text
    )
    ...
    while num_frames < args.frames:
        ...
        if args.early_stop_reward is not None:
            mean_reward = np.mean(logs["return_per_episode"])
            if mean_reward >= args.early_stop_reward:
                txt_logger.info(
                    f"\nEarly stopping: mean reward {mean_reward:.2f} >= threshold {args.early_stop_reward}"
                )
                break
